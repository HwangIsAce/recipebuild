...
def main():
    ...
    if training_args.do_eval:
        ...
        def compute_metrics(eval_preds):
            ...
            logger.info(f'Label most  = {l_cnt.most_common(5)}')
            logger.info(f'Pred  most  = {p_cnt.most_common(5)}')
            logger.info(f'Label least = {l_cnt.most_common()[-5:]}')
            logger.info(f'Pred  least = {p_cnt.most_common()[-5:]}')
            
            import random
            _lset = set(labels[:200])
            _random_ids = random.sample(_lset, 10)
            for _ids in _random_ids:
                logger.info(f'{_ids:5d}:label_cnt = {l_cnt[_ids]:5d}, pred_cnt = {p_cnt[_ids]:5d}')
